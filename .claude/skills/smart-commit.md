...
| 현재 브랜치 | push 가능? | 조치 |
|-------------|------------|------|
| `master` | 금지 | "master에 직접 push할 수 없습니다. feature 브랜치를 생성하세요." 안내 후 **중단** |
| `develop` | 금지 | "develop에 직접 push할 수 없습니다. feature 브랜치를 생성하세요." 안내 후 **중단** |
| `feature/*`, `fix/*`, `hotfix/*` | 허용 | 계속 진행 |

**master/develop에 있는 경우 새 브랜치 생성 제안**:
```bash
...
브랜치 이름에서 Issue 번호를 추출:
- `feature/1-db-schema` → Issue #1
- `fix/5-matching-logic` → Issue #5
...
if [ -n "$DETECTED_ISSUE_NUM" ]; then
  echo -e "\n\nCloses #$DETECTED_ISSUE_NUM" >> .pr_body_temp.md
fi
```
...
1. **master/develop에 직접 push 금지** - feature 브랜치 사용 필수
